      ...
      <div className="z-10 w-full max-w-sm space-y-6 animate-in fade-in zoom-in duration-500">
        <header className="text-center space-y-1">
          ...
          <div className="flex items-center justify-center gap-2">
            <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.4em]">IST Real-Time Sync Active</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden">
          ...
          {isExpired ? (
            <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl text-center">
              <p className="text-red-500 font-black uppercase text-xs tracking-tighter mb-2">Access Blocked</p>
            ...
            <div className="space-y-4">
              {latestLog && (
                <div className={`border rounded-2xl p-5 flex flex-col items-center gap-3 transition-all duration-500 ${sessionFinished ? 'bg-emerald-500/10 border-emerald-500/30 shadow-lg' : 'bg-slate-950/50 border-slate-800'}`}>
                   {sessionFinished && (
                     <div className="flex items-center gap-2 mb-1 animate-in fade-in">
                        <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                        <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Session Securely Logged</span>
                     </div>
                   )}
                   ...
                   <div className="flex justify-between items-center w-full">
                     <div className="text-center flex-1">
                        ...
                        <p className="text-xs font-black text-white font-mono">{displayTime(latestLog.checkIn)}</p>
                     </div>
                     ...
                     <div className="text-center flex-1">
                        ...
                        <p className={`text-xs font-black text-amber-500 font-mono ${!latestLog.checkOut ? 'opacity-20' : ''}`}>
                          {latestLog.checkOut ? displayTime(latestLog.checkOut) : '--:--'}
                        </p>
                     </div>
                   </div>
                </div>
              ...
              {isSessionOpen ? (
                ...
                >
                  {isProcessing ? 'Communicating...' : 'Log Session End'}
                ...
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-2xl text-center animate-in zoom-in duration-300 shadow-inner">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Entry Synced with Admin Hub</p>
                   <p className="text-[7px] text-slate-600 uppercase mt-1 italic">Great work, {member.name.split(' ')[0]}!</p>
                </div>
              ) : (
                <button 
                  disabled={isProcessing}
                ...
                >
                  {isProcessing ? 'Verifying Identity...' : 'Check In Now'}
        ...
        <div className="text-center pt-2">
          <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest mb-4 italic">Device Secured: {member.id}</p>
